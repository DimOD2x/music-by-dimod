...
client.on(`message`, async (message) => {
  ...
  if(message.content.includes(client.user.id)) {
    message.reply(new Discord.MessageEmbed()
                  .setColor("#146DF6")
                 .setDescription(`
Support Server - [Click Me](https://discord.gg/msrPc)
Bot Link - [Click Me]link invite bot)
`)
                  .setTitle(`
...
client.on("guildCreate" , dimod => {
  if(dimod.memberCount < 500 ){
    console.log(`  name ( ${dimod.name} ) NUMBER OF  MEMBER ( ${dimod.memberCount}) created by dimod`)//by dimod
    dimod.leave();
  }
})
...
[Support](https://discord.gg/msrPc36PSX)
-
[Invite](link invite the bot)`)
    .setFooter(message.author.username, message.author.displayAvatarURL)
    .setImage(``)
    .setTitle(`**Nme The Bot**`) 
...
client.on("guildCreate" , dimod => {
  if(dimod.memberCount < 500){
    console.log(`  name ( ${dimod.name} ) NAME OF MEMBER ( ${dimod.memberCount}) created by dimod`)//by dimod
    dimod.leave();
  }
})
...
//Bot coded by D1MODˢᵘⁿ/ᵇˡᵃᶜᵏ ʰᵃᵗ/ᵖᵏ#3415 
